...
def projects_list(request: HttpRequest, category_slug=None):
    ...
    try:
        ...
        logger.info(
            f"Stats - Total: {total_projects}, Completed: {completed_projects}, In Progress: {in_progress_projects}"
        )
...
def project_detail(
    request: HttpRequest, project_slug: str = None, project_id: int = None
):
    ...
    try:
        ...
        if not request.session.get(session_key, False):
            ...
            logger.info(
                f"View count updated for project {project.id}: {project.view_count}"
            )
        else:
            logger.info(f"Project {project.id} already viewed in this session")

        related_projects = (
            Projects.objects.filter(category__in=project.category.all())
        ...
        if not related_projects.exists():
            related_projects = Projects.objects.exclude(id=project.id).order_by(
                "-created_date"
            )[:3]
            logger.info("No related projects by category, using latest projects")

        featured_projects = Projects.objects.filter(featured=True).exclude(
            id=project.id
        )[:3]
