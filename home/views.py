    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                logger.info(f'Contact form saved successfully: {contact.name} - {contact.email}')
                logger.info('Everything is OK!')
                messages.success(request, "فرم با موفقیت ارسال شد!")
                return redirect('home:home')
            except Exception as e:
                logger.error(f'Error saving contact form: {e}')
                messages.error(request, "خطا در ذخیره فرم. لطفاً دوباره تلاش کنید.")
                return redirect('home:home')
        else:
...
def profile_view(request:HttpRequest):
    ...
    try:
        ...
        logger.info(f'Profile does not exist for user {request.user.username}, creating new profile')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.error(request, 'خطا در به‌روزرسانی پروفایل. لطفاً دوباره تلاش کنید.')
        else:
